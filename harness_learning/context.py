...
class ContextManifest:
    query_id: str; total_byte_budget: int; used_bytes: int; ablations: dict; layers: dict; selected_records: dict
...
class ContextComposer:
    ...
    def compose(self,request,references,episodes,skills):
        ...
        selected_records={name:tuple(request.mandatory_layers.get(name,())) for name in ("L0","L1","L2")}
        ...
        for name,records,enabled,kind in (("L3",references,True,"reference"),("L4",episodes,request.enable_l4,"episode"),("L5",skills,request.enable_l5,"skill")):
            ...
            if not enabled:
                layers[name]=LayerManifest(name,False,item_limit,byte_limit,0,0,(),{getattr(r,"id",str(i)):"ablated" for i,r in enumerate(records)}); selected_records[name]=(); continue
            if records:
                ...
                ids=tuple(h.record_id for h in result.hits); layer_used=result.used_bytes; reasons=result.exclusion_reasons; selected_records[name]=tuple(h.record for h in result.hits)
            else: ids=(); layer_used=0; reasons={}; selected_records[name]=()
            layers[name]=LayerManifest(name,True,item_limit,byte_limit,len(ids),layer_used,ids,reasons); used+=layer_used
        return ContextManifest(stable_id("query",{"text":request.text,"kind":request.task_kind}),self.total_byte_budget,used,{"enable_l4":request.enable_l4,"enable_l5":request.enable_l5},layers,selected_records)
